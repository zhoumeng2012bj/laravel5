<template>
	<div>
		<div>
			<el-button type="primary" class="el-icon-plus" @click="addFollow"> 新 增</el-button>
		</div>
...
<script>
	import {pressmoneyPurchaseContract,addpressmoneyPurchaseContract} from  '../../api/api';
  export default {
    data() {
      return {
		...
		methods:{
			...
			purchaseContractList() {
				...
				pressmoneyPurchaseContract(para1).then((res)=>{
			...
			formFollowsure(){
				...
				this.formFollow.cwid = this.cwid;
				let para = Object.assign({}, this.formFollow);
				addpressmoneyPurchaseContract(para).then((res)=>{
						if(res.data.code == 200)　{
							this.dialogFormVisibleFollow = false;
							location.reload();
						}else{
